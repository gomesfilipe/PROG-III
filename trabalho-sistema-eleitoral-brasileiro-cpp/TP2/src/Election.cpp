...
Election::Election(vector<Candidate*>& candidates, const vector<PoliticParty*>& partys, const string& date) {
...
void Election::report_1() const {
    cout << "Número de vagas: " << this->qtd_elected() << endl;
    cout << endl;
}
...
void Election::report_5() {
    ...
    for(Candidate* p : this->candidates) {
        if (elect_proportional == qtd_elect) {
            break;
        }
...
void Election::report_11() const {
    ...
    cout << "Total de votos nominais:   " <<  nominal_votes <<  " ("  << formatDoubleCurrency(nominal_percent, LOCALE_PT_BR) << "%)" << endl;
